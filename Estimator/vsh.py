...
def del_Ylm(l, m, theta, phi):
    dYdtheta = (m*(1/np.tan(theta))*Ylm(l, m, theta, phi) 
                + np.sqrt((l-m)*(l+m+1))*np.exp(-1.j*phi)*Ylm(l, m+1, theta, phi))

    dYdphi = 1.j*m*Ylm(l, m, theta, phi)

    return np.array([dYdtheta,(1/(np.sin(theta)))*dYdphi])
...
def Psilm_vec(l, m, r, theta, phi):
    ...
    """
    if m > 0:
        return del_Ylm(l, m, theta, phi)/np.sqrt((l*(l+1)))
    else:
