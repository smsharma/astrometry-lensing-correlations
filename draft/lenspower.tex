...
% \begin{align}
% \left\langle \dot{\Phi}(\vect{y}) \dot{\Phi}(\vect{y}') \right\rangle &= (4\pi \GN)^2 \int \ddbar^3 k \, \ddbar^3 k' \, \frac{e^{i(\vect{k}\cdot \vect{y} + \vect{k}' \cdot \vect{y}')}}{\vect{k}^2 \vect{k}^{\prime 2}}\\
...
% &\simeq \frac{\sqrt{\pi}}{8} (4 \pi \GN)^2 n_0^2 \frac{\mathrm{erf}\left(\sigma_k |\vect{y}-\vect{y}'|\right)}{\sigma_k |\vect{y}-\vect{y}'|}
% \end{align}
...
% &\hspace{0em} \simeq \frac{\sqrt{\pi}}{2} (4\pi \GN)^2 \frac{\rho_0^2 \sigma_v D}{m} F^{(\mu)}\left[ |\vect{\theta}-\vect{\theta}'|,\sigma_k D \right] \nonumber \\
...
% &= \Delta \Omega \, \pi^{3/2} (4\pi \GN)^2 \frac{\rho_0^2 \sigma_v D}{m} G^{(\mu)}\left[ \ell, \sigma_k D \right] \nonumber
% \end{align}
...
%For an isotropically distributed and homogeneous lens population, the signal $\langle |\mu_{\ell m}^{(1)}|^2 \rangle$ will therefore be an \emph{increasing} function of $|m|/\ell$ at fixed $\ell$.
